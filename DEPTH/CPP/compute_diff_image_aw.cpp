...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //mexPrintf("dimxA=%d, dimyA=%d", dimxA, dimyA);
    ...
    double g1 = 255; // sigma for distances
    double g2 = 100.0; // sigma for intensity (if range changes (not 0-255 but 0-1) change accordingly)
    ...
    for(i=hws+1;i<dimxA-hws-1;i++) // 350; i<400; i++) //
    {
        for(j=hws+1;j<dimyA-hws-1;j++) // 350; j<400; j++) //
        {
            ...
            for(h=-hws;h<hws;h++)
            {
                for(w=-hws;w<hws;w++)
                {
                    mean_p1 += aif[(i+h)*dimyA+(j+w)];
                    mean_p2 += cur_fs[(i+h)*dimyA+(j+w)];
                }
            }
            mean_p1 /= (ws*ws);
            mean_p2 /= (ws*ws);
            ...
            for(h=-hws;h<hws;h++)
            {
                for(w=-hws;w<hws;w++)
                {
                    ...
                    weight = exp(-(sqrt((h*h)+(w*w))/g1 + std::abs(aif[(i+h)*dimyA+(j+w)] - cur_fs[(i+h)*dimyA+(j+w)]) / g2));
                    ...
                    sad_sum += weight * std::min(TRUNC_AD, std::abs(aif[(i+h)*dimyA+(j+w)] - cur_fs[(i+h)*dimyA+(j+w)]));
                    weight_sum += weight;
                }
            }   
            sigma_p1 /= (ws*ws);
            ...
            output[i*dimyA+j] = (alpha)*norm_NCC + (1-alpha)*norm_SAD; //-cur_fs[i*dimy1+j]; 
        }
    }
