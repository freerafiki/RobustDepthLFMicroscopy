...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //outputs
    plhs[0] = mxCreateDoubleMatrix(dimxA, dimyA, mxREAL);
    ...
    double weight_dist = 0.0; // weight distance-based
    double weight_int = 0.0; // weight intensity-based
        ...
        for(j=hws+1;j<dimyA-hws-1;j++) // 350; j<400; j++) //
        {
            ...
            for(h=-hws;h<hws;h++)
            {
                for(w=-hws;w<hws;w++)
                {
                    ...
                    sad_sum += std::min(TRUNC_AD, std::abs(aif[(i+h)*dimyA+(j+w)] - cur_fs[(i+h)*dimyA+(j+w)]));
