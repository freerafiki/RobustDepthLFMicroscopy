...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for(i=100;i<200;i++) //pad+hws; i<xL-pad-hws; i++) //dmax+hws+1;i<s1-dmax-hws-1;i++) //  //(i=hws+1;i<s1-hws-1;i++)
    {
        for(j=200;j<300;j++) //pad+hws; j<yL-pad-hws; j++) //dmax+hws+1;j<s2-dmax-hws-1;j++) //    (j=hws+1;j<s2-hws-1;j++)
        {       
            for(h=-hws;h<hws;h++)
            {
                for(w=-hws;w<hws;w++)
            ...
            {   
                ...
                else
                {
                    ...
                    leftVal = imgL[i-i_+(j-j_)*xL];
                    rightVal = imgR[i+i_+(j+j_)*xL];
                    for(h=-hws;h<hws;h++)
                    {
                        for(w=-hws;w<hws;w++)
                        {
                            // build census windows
                            if (imgR[i+i_+h+(j+j_+w)*xL] < imgR[i+i_+(j+j_)*xL])
                                right_census[h+hws][w+hws] = 0;
                            else
                                right_census[h+hws][w+hws] = 1;
                            if (imgL[i-i_+h+(j-j_+w)*xL] < imgL[i-i_+(j-j_)*xL])
                                left_census[h+hws][w+hws] = 0;
                            else
                                left_census[h+hws][w+hws] = 1;

                            //weights --> using adaptive support windows
                            diffR = std::min(TRUNC_AD, std::abs(imgC[i+h+(j+w)*xL] - imgR[i+i_+h+(j+j_+w)*xL]));
                            diffL = std::min(TRUNC_AD, std::abs(imgC[i+h+(j+w)*xL] - imgL[i-i_+h+(j-j_+w)*xL]));
                ...
                cost_volume_L[i + j * xL + dd * (xL * yL)] = norm_SAD_L*alpha+norm_CEN_L*(1-alpha);
                cost_volume_R[i + j * xL + dd * (xL * yL)] = norm_SAD_R*alpha+norm_CEN_L*(1-alpha);
            }
        }
    }
