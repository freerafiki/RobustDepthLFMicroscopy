...
function [cv, depthmap, conf] = hex_stereo_matching(EIs, c_img, ws, alpha, d_min, d_max, matte, gamma1, gamma2, priors, use_failure_prediction)
    ...
    for m = 1: size(cvL1, 3)
        ...
        cur_slice_L1 = high_freqs .* (slice_L1_s0 .* 0.6 + slice_L1_s1 .* 0.3 + slice_L1_s2 .* 0.1) + ...
       	...
       	cur_slice_L1 = slice_L1_s0 .* (gamma1 .* high_freqs) + slice_L1_s0 .* (gamma2 .* med_freqs) + ...
            slice_L1_s1 .* (gamma2 .* (high_freqs + low_freqs)) + slice_L1_s1 .* (gamma1 .* med_freqs) + ...
            slice_L1_s2 .* (gamma2 .* med_freqs) + slice_L1_s2 .* (gamma1 .* low_freqs);
        cur_slice_L2 = slice_L2_s0 .* (gamma1 .* high_freqs) + slice_L2_s0 .* (gamma2 .* med_freqs) + ...
            slice_L2_s1 .* (gamma2 .* (high_freqs + low_freqs)) + slice_L2_s1 .* (gamma1 .* med_freqs) + ...
            slice_L2_s2 .* (gamma2 .* med_freqs) + slice_L2_s2 .* (gamma1 .* low_freqs);
        cur_slice_L3 = slice_L3_s0 .* (gamma1 .* high_freqs) + slice_L3_s0 .* (gamma2 .* med_freqs) + ...
            slice_L3_s1 .* (gamma2 .* (high_freqs + low_freqs)) + slice_L3_s1 .* (gamma1 .* med_freqs) + ...
            slice_L3_s2 .* (gamma2 .* med_freqs) + slice_L3_s2 .* (gamma1 .* low_freqs);
        cur_slice_R1 = slice_R1_s0 .* (gamma1 .* high_freqs) + slice_R1_s0 .* (gamma2 .* med_freqs) + ...
            slice_R1_s1 .* (gamma2 .* (high_freqs + low_freqs)) + slice_R1_s1 .* (gamma1 .* med_freqs) + ...
            slice_R1_s2 .* (gamma2 .* med_freqs) + slice_R1_s2 .* (gamma1 .* low_freqs);
        cur_slice_R2 = slice_R2_s0 .* (gamma1 .* high_freqs) + slice_R2_s0 .* (gamma2 .* med_freqs) + ...
            slice_R2_s1 .* (gamma2 .* (high_freqs + low_freqs)) + slice_R2_s1 .* (gamma1 .* med_freqs) + ...
            slice_R2_s2 .* (gamma2 .* med_freqs) + slice_R2_s2 .* (gamma1 .* low_freqs);
        cur_slice_R3 = slice_R3_s0 .* (gamma1 .* high_freqs) + slice_R3_s0 .* (gamma2 .* med_freqs) + ...
            slice_R3_s1 .* (gamma2 .* (high_freqs + low_freqs)) + slice_R3_s1 .* (gamma1 .* med_freqs) + ...
            slice_R3_s2 .* (gamma2 .* med_freqs) + slice_R3_s2 .* (gamma1 .* low_freqs);

        % apply matte and guided filtering
        cvL1msc(:,:,m) = max(0, guidedfilter_color_runfilter(cur_slice_L1 .* double(matte)));
        cvL2msc(:,:,m) = max(0, guidedfilter_color_runfilter(cur_slice_L2 .* double(matte)));
        cvL3msc(:,:,m) = max(0, guidedfilter_color_runfilter(cur_slice_L3 .* double(matte)));
        cvR1msc(:,:,m) = max(0, guidedfilter_color_runfilter(cur_slice_R1 .* double(matte)));
        cvR2msc(:,:,m) = max(0, guidedfilter_color_runfilter(cur_slice_R2 .* double(matte)));
        cvR3msc(:,:,m) = max(0, guidedfilter_color_runfilter(cur_slice_R3 .* double(matte)));
    ...
    if use_failure_prediction 
        ...
        fail1_map = reshape(fail1, [size(epi1_fail_map,1), size(epi1_fail_map,2)]) ./ quantiz_levels;
        
        vec_2 = reshape(abs(epi2_fail_map), size(epi2_fail_map, 1) * size(epi2_fail_map, 2), 1);
        fail2 = Quantiz(vec_2, 0:step:max_val);
        fail2_map = reshape(fail2, [size(epi2_fail_map,1), size(epi2_fail_map,2)]) ./ quantiz_levels;
        
        vec_3 = reshape(abs(epi3_fail_map), size(epi3_fail_map, 1) * size(epi3_fail_map, 2), 1);
        fail3 = Quantiz(vec_3, 0:step:max_val);
        fail3_map = reshape(fail3, [size(epi3_fail_map,1), size(epi3_fail_map,2)]) ./ quantiz_levels;
    end
    ...
    for k = 1: size(cvL1msc,3)
        ...
        if use_failure_prediction
            cv(:,:,k) = cvL1msc(:,:,k) .* fail1_map + cvR1msc(:,:,k) .* fail1_map + ...
                cvL2msc(:,:,k) .* fail2_map + cvR2msc(:,:,k) .* fail2_map + ...
                cvL3msc(:,:,k) .* fail3_map + cvR3msc(:,:,k) .* fail3_map;
        else
